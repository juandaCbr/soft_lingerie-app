    ...
    try {
      ...
      if (prodData) {
        ...
        prodData.forEach(item => {
          const catNombre = typeof item.categoria === 'object' ? item.categoria?.nombre : item.categoria;
          if (catNombre) tempCats.add(String(catNombre).trim());
        ...
        setCategorias(Array.from(tempCats));
      }
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 md:px-8 py-10 text-[#4a1d44] min-h-screen relative">
      ...
      {showScrollTop && (
        <button
          onClick={irArriba}
          className="fixed bottom-8 right-8 z-50 p-4 bg-[#4a1d44] text-white rounded-full shadow-2xl transition-all hover:scale-110 active:scale-90 animate-in fade-in zoom-in"
        >
          <ArrowUp size={20} />
        </button>
      )}
      ...
      <header className="text-center mb-12 flex flex-col items-center">
        <span className="text-[10px] font-black uppercase tracking-[0.5em] text-[#4a1d44]/30 mb-4">
          Colecciones Exclusivas
        </span>
        <h1 className="text-4xl md:text-6xl font-black font-playfair uppercase text-[#4a1d44] tracking-tight italic">
          Catálogo Soft
        </h1>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 bg-white/60 p-5 rounded-[2rem] border border-[#4a1d44]/5 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-[#f2e1d9] flex items-center justify-center">
            <SlidersHorizontal size={14} className="text-[#4a1d44]" />
          </div>
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="flex items-center gap-2 bg-white px-5 py-2.5 rounded-2xl border border-[#4a1d44]/5 shadow-sm w-full md:min-w-[200px]">
            ...
            <select
              ...
              className="bg-transparent text-xs font-bold outline-none cursor-pointer w-full text-[#4a1d44]"
            >
            ...
            <div className="mb-8">
              <div className="relative group">
                <input
                  ...
                  className="w-full bg-[#fdf8f6] border border-transparent focus:border-[#4a1d44]/10 py-4 pl-5 pr-12 rounded-2xl outline-none text-sm font-medium transition-all"
                  value={busqueda}
              ...
              {isColorFilterOpen && (
                <div className="grid grid-cols-5 gap-2.5 pt-2 animate-in fade-in slide-in-from-top-2 duration-300">
                  <button
                    ...
                    className={`w-9 h-9 rounded-full border flex items-center justify-center text-[8px] font-black ${colorSeleccionado === 'Todos' ? 'border-[#4a1d44] bg-[#4a1d44] text-white shadow-lg' : 'border-gray-100 hover:bg-gray-50'}`}
                  >
                    TODO
                  </button>
      ...
      <section className="mt-10 mb-20 bg-[#4a1d44] text-white p-10 md:p-16 rounded-[3.5rem] shadow-2xl relative overflow-hidden text-center md:text-left">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="max-w-xl">
            ...
            <p className="text-sm md:text-lg opacity-70 font-medium leading-relaxed">
              No te preocupes, estamos aquí para asesorarte de forma personalizada por WhatsApp.
              ¡Escríbenos y encuentra tu conjunto ideal!
            </p>
          </div>
          <a
            href={`https://wa.me/${numeroWhatsApp}?text=${encodeURIComponent("¡Hola! Estoy viendo el catálogo y necesito asesoría con una prenda.")}`}
            target="_blank"
            className="group flex items-center gap-4 bg-white text-[#4a1d44] px-10 py-5 rounded-full font-bold text-sm uppercase tracking-widest hover:bg-[#f2e1d9] transition-all shadow-xl whitespace-nowrap active:scale-95"
          >
